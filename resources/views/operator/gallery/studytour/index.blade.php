...
@section('title', 'Data Album Study Tour SDN Caringin Ngumbang')
...
@section('content')
    <div class="container">
        ...
        <div class="table-responsive">
            <table id="tablePariwisata" class="table table-bordered table-hover table-striped">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Nama</th>
    ...
    @push('js')
        ...
        <script>
            ...
            function dataPariwisata() {
                $('#tablePariwisata').DataTable({
                    serverSide: true,
                    responsive: true,
                    processing: true,
                    ajax: {
                        url: "{{ route('gallery-pariwisata.index') }}",
                        type: 'GET'
                    },
                    columns: [
                        {
                        ...
                        },
                        {
                            ...
                            name: 'gambar',
                            render: function (data, type, row) {
                                let photoUrl = `/images/gallery/studytour/${data}`;
                        ...
                        },
                        {
                            data: null,
                            render: function (data, type, row) {
                                let editUrl = `{{ route('gallery-pariwisata.edit', ':id') }}`.replace(':id', row.id);
                                let deleteUrl = `{{ route('gallery-pariwisata.destroy', ':id') }}`.replace(':id', row.id);
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                Swal.fire({
                                    ...
                                    $('#tablePariwisata').DataTable().ajax.reload();
                                });
                            },
                            error: function(xhr) {
                                Swal.fire({
